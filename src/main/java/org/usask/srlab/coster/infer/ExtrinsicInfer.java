...
public class ExtrinsicInfer {
    ...
    public static void extrinsicInference(int topk){
        print("Collecting Jar files and Test Projects");
        logger.info("Collecting Jar Files and Test Projects");
        String[] jarPaths = ParseUtil.collectGithubJars(new File(Config.SO_JAR_PATH));
        print("Collecting SO code snippets");
        logger.info("Collecting SO code snippets");
        ArrayList<String> snippetPaths = ParseUtil.collectSOSnippets(new File(Config.SO_CODE_SNIPPET_PATH));
        String[] sourcefilePaths = new String[snippetPaths.size()];
        sourcefilePaths = snippetPaths.toArray(sourcefilePaths);
        logger.info("Total Number of SO Code Snippet: "+ sourcefilePaths.length);

        print("Collecting data set from the SO datasets");
        logger.info("Collecting dataset from the SO datasets");
        List<APIElement> testCases = InferUtil.collectSODataset(sourcefilePaths,jarPaths);
